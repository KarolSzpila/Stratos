...
static void USBH_Process_OS(void const * argument)
{
  ...
  for(;;)
  {
    ...
    {
      USBH_HandleTypeDef* usbHandler = (USBH_HandleTypeDef *)argument;
      USBH_Process(usbHandler);

      if(usbHandler->gState == HOST_CHECK_CLASS) {

          						 // attempt a connection on the control endpoint
          						 // note: max packet size 64 bytes for FS and 512 bytes for HS
          						 InPipe = USBH_AllocPipe(usbHandler, USB_PIPE_NUMBER);
          						 USBH_StatusTypeDef status = USBH_OpenPipe(usbHandler,
          													 InPipe,
          													 USB_PIPE_NUMBER,
															 usbHandler->device.address,
															 usbHandler->device.speed,
          													 USB_EP_TYPE_BULK,
          													 512);
