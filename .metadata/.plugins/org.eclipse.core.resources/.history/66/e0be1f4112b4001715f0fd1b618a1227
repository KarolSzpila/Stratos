...
RTLSDR_TunerTypeDef  Tuner_R82xx =
{
	"R82xx",
	R82XX_Init,
	R82XX_InitProcess,
...
USBH_StatusTypeDef R82XX_Init(USBH_HandleTypeDef *phost)
{
	 ...
	 RTLSDR_Handle->tuner->tunerData =
	       (R82XX_HandleTypeDef *)USBH_malloc (sizeof(RTLSDR_HandleTypeDef));

	 R82XX_HandleTypeDef * R82XX_Handle =
	     (R82XX_HandleTypeDef*) RTLSDR_Handle->tuner->tunerData;

	  R82XX_Handle->initState = R82XX_REQ_RUN;
	  R82XX_Handle->initNumber = 0;
	  R82XX_Handle->maskState = R82XX_MASK_READ;
	  R82XX_Handle->gainState = 0;
	  R82XX_Handle->ifGainState = 0;
	  R82XX_Handle->iffiltState=0;
	  R82XX_Handle->bandSetState=0;
	  R82XX_Handle->tuneFreqState=0;
	  R82XX_Handle->tuneParamsState=0;
	  R82XX_Handle->vco.fosc = DEF_RTL_XTAL_FREQ;
	  R82XX_Handle->setBWState=0;
	  R82XX_Handle->corr=0;
	  return USBH_OK;
}
...
USBH_StatusTypeDef R82XX_InitProcess(USBH_HandleTypeDef *phost)
{
	  ...
	  uint32_t rtl_xtal;
	  int32_t if_freq;
	  uint32_t freq = R82XX_IF_FREQ;
	  rtlsdr_get_xtal_freq(R82XX_Handle,&rtl_xtal, NULL);
	  if_freq = ((freq * TWO_POW(22)) / rtl_xtal) * (-1);

	  uint8_t tmp;
	  switch (R82XX_Handle->initState) {

	  case R82XX_REQ_RUN:
		  switch(R82XX_Handle->initNumber) {

		  case 0: uStatus = RTLSDR_demod_write_reg(phost, 1, 0xb1, 0x1a, 1); break;

		  case 1: uStatus = RTLSDR_demod_write_reg(phost, 0, 0x08, 0x4d, 1); break;

		  case 2:
			  tmp = (if_freq >> 16) & 0x3f;
			  uStatus = RTLSDR_demod_write_reg(phost, 1, 0x19, tmp, 1); break;
			  break;

		  case 3:
			  tmp = (if_freq >> 8) & 0xff;
			  uStatus = RTLSDR_demod_write_reg(phost, 1, 0x1a, tmp, 1); break;
			  break;

		  case 4:
			  tmp = if_freq & 0xff;
			  uStatus = RTLSDR_demod_write_reg(phost, 1, 0x1b, tmp, 1); break;
			  break;

		  case 5: uStatus = RTLSDR_demod_write_reg(phost, 1, 0x15, 0x01, 1); break;

		  case 6:
			  R82XX_Handle->xtal_cap_sel = XTAL_HIGH_CAP_0P;
			  StoreShadow(R82XX_Handle,0x05,r82xx_init_array, sizeof(r82xx_init_array));
			  int rc, size, pos = 0;
			  shadow_store(priv, reg, val, len);
			  do{
			  		size = len;
			  		...
			  		uStatus = RTLSDR_i2c_write_reg(phost,IICB,reg,&val[pos]);
			  		if(uStatus != USBH_OK)
			  		{
			  			return;
			  		}
		  ...
		  if (uStatus == USBH_OK) {
		         ...
		         R82XX_Handle->initState = R82XX_REQ_INC;
		    ...
		    case R82XX_REQ_INC:
		      ...
		      if (R82XX_Handle->initNumber == 32) {
		    	  R82XX_Handle->initState = R82XX_REQ_COMPLETE;
		    	  R82XX_Handle->initNumber = 0;
		      } else {
		    	  R82XX_Handle->initNumber++;
		    	  R82XX_Handle->initState = R82XX_REQ_RUN;
		      }
		      retStatus = USBH_BUSY;
		      break;
...
#define APPLY_PPM_CORR(val,ppm) (((val) * (1.0 + (ppm) / 1e6)))
    ...
    if (rtl_freq)
    { *rtl_freq = (uint32_t) APPLY_PPM_CORR(DEF_RTL_XTAL_FREQ, R82XX_Handle->corr); }

    if (tuner_freq)
    { *tuner_freq = (uint32_t) APPLY_PPM_CORR(DEF_RTL_XTAL_FREQ, R82XX_Handle->corr); }

}

void StoreShadow(R82XX_HandleTypeDef* priv, uint8_t reg, const uint8_t *val,int len)
{
	int r = reg - REG_SHADOW_START;
